...
namespace STSdb4.Remote.Commands
{
    public partial class CommandPersist
    {
        ...
        private void WriteStorageEngineSizeCommand(BinaryWriter writer, ICommand command)
        {
            var cmd = (StorageEngineDatabaseSizeCommand)command;
            writer.Write(cmd.Size);
        }

        private StorageEngineDatabaseSizeCommand ReadStorageEngineSizeCommand(BinaryReader reader)
        {
            return new StorageEngineDatabaseSizeCommand(reader.ReadInt64());
        }
