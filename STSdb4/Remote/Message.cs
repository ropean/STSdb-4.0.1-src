...
namespace STSdb4.Remote
{
    ...
    public class Message
    {
        ...
        public void Serialize(BinaryWriter writer)
        {
            ...
            CommandPersist persist = ID > 0 ? new CommandPersist(new DataPersist(Description.KeyType), new DataPersist(Description.RecordType)) : new CommandPersist(null, null);
        ...
        public static Message Deserialize(BinaryReader reader, Func<long, IDescriptor> find)
        {
            ...
            if (ID > 0)
            {
                try
                {
                    ...
                    persist = new CommandPersist(new DataPersist(description.KeyType), new DataPersist(description.RecordType));
                }
                catch (Exception exc)
                {
