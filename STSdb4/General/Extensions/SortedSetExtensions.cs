...
namespace STSdb4.General.Extensions
{
    public class SortedSetHelper<T>
    {
        ...
        {
            LambdaFind = RunningOnMono ? CreateMonoFindMethod() : CreateFindMethod();
            find = LambdaFind.Compile();

            internalMonoConstructFromSortedArray = RunningOnMono ? CreateInternalConstructFromSortedArray().Compile() : null;
            LambdaConstructFromSortedArray = RunningOnMono ? CreateMonoConstructFromSortedArray() : CreateConstructFromSortedArrayMethod();
            constructFromSortedArray = LambdaConstructFromSortedArray.Compile();

            LambdaReplace = RunningOnMono ? CreateMonoReplaceMethod() : CreateReplaceMethod();
            replace = LambdaReplace.Compile();

            LambdaGetViewBetween = RunningOnMono ? CreateMonoGetViewBetweenMethod() : CreateGetViewBetweenMethod();
